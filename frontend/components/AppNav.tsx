  ...
  return (
    <nav style={{
      ...
      <div style={{ padding: "0 18px 20px", borderBottom: "1px solid var(--border-subtle)" }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: "var(--text-primary)", letterSpacing: "0.04em", lineHeight: 1.2 }}>
          THE TRADING FLOOR
        </div>
      ...
      <div style={{ flex: 1, padding: "16px 0" }}>
        {NAV_GROUPS.map((group) => (
          <div key={group.group} style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-tertiary)", textTransform: "uppercase", letterSpacing: "0.12em", padding: "0 18px", marginBottom: 6 }}>
              {group.group}
            </div>
            {group.items.map(({ href, label, icon, badge }) => {
              ...
              return (
                <Link key={href} href={href as never} style={{
                  ...
                  <span style={{ fontSize: 14, width: 18, textAlign: "center", flexShrink: 0, color: isActive ? "var(--accent-info)" : "var(--text-tertiary)" }}>
                    {icon}
                  </span>
                  {label}
                  {badge && (
                    <span style={{ marginLeft: "auto", fontSize: 9, fontWeight: 700, background: "rgba(99,102,241,0.2)", color: "var(--accent-info)", padding: "1px 5px", borderRadius: 3, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                      {badge}
                    </span>
                  )}
                </Link>
      ...
      <div style={{ padding: "14px 18px 0", borderTop: "1px solid var(--border-subtle)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--accent-profit)", boxShadow: "0 0 5px var(--accent-profit)" }} />
        ...
        <div style={{ fontSize: 10, color: "var(--text-tertiary)", lineHeight: 1.6 }}>
          Crypto · Commodities<br />Copy Trade Active
        </div>
      </div>
    </nav>
  );
