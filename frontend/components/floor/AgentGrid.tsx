  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 20 }}>
        <p style={{ color: "var(--text-tertiary)", fontSize: 12, margin: 0, maxWidth: 700 }}>
          Each card shows live status, current task, ELO rating, and the last known signal. Cycles run automatically — use <strong>Trigger Cycle Now</strong> to fire one on-demand.
        </p>
        <button onClick={runFullCycle} disabled={running === "cycle"} style={{
          ...
          padding: "8px 18px", fontSize: 12, fontWeight: 600, cursor: running === "cycle" ? "not-allowed" : "pointer",
        }}>
          {running === "cycle" ? "Running…" : "Trigger Cycle Now"}
        </button>
      </div>
